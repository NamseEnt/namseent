...
semantic action mode는 checkpoint hyperparameter에도 기록된다. `validate`, `diagnostic-trace`, `overfit-gate`, resume은 checkpoint와 요청한 `--semantic-actions`가 다르면 중단하므로 레거시와 semantic candidate space를 조용히 섞지 않는다.
