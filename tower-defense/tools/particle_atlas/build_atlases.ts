...
export async function createAttackAtlas(): Promise<Atlas> {
    const attack = createAtlas("attack", CELL * 14, CELL);
    await drawImage(
        attack,
        "BURNING_TAIL",
    ...
    await drawImage(
        attack,
        "BLACK_SMOKE_00",
        path.join(ASSET_DIR, "attack", "particle", "black_smoke_00.png"),
    );
    await drawImage(
        attack,
        "BLACK_SMOKE_01",
    ...
    await drawImage(
        attack,
        "YELLOW_EXPLOSION",
        path.join(ASSET_DIR, "attack", "particle", "yellow_explosion.png"),
    );
    return attack;
}
