...
impl Component for PreviewEntryComponent {
    fn render(self, ctx: &RenderCtx) {
        ...
        let core_upgrades = upgrade_state.to_core_state();
        let core_template = template.to_core_state();

        let tracked_upgrade_revision = ctx.track_eq(&upgrade_state.revision);
        ...
        let dps_text_signal = ctx.memo(|| {
            ...
            let damage_multiplier =
                FixedRatio::from_raw(FixedRatio::ONE.raw().saturating_add(
                    core_upgrades.tower_damage_bonus_raw_for_template(&core_template),
                ));
            let damage = template.calculate_rating(damage_multiplier);
