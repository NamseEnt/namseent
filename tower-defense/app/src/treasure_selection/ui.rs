...
impl Component for TreasureSelectionUi {
    fn render(self, ctx: &RenderCtx) {
        ...
        let pending_selection = match game_state.raw_core_state().flow() {
        ...
        let presentation_flow = game_state.state().presentation_flow_snapshot();
        let flow = if let GameFlow::TreasureSelection(flow) = &presentation_flow {
            flow
        } else {
            return;
        };
        ...
        let is_closing = pending_selection.is_some();
        ...
        if let Some(selected_index) = pending_selection
            && modal_scale < 0.02
        {
            dispatch_player_command(PlayerCommand::SelectTreasure {
