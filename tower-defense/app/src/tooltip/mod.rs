...
use crate::game_state::upgrade::{Upgrade, UpgradePresentation};
    ...
    fn sections(
        ...
        match self {
            ...
            TooltipContent::Shop { content, slot_id } => {
                let mut sections = content.sections(locale, game_state);
                let status = game_state.shop_purchase_status(*slot_id);
    ...
    let reason_texts = reasons
        ...
        .map(|reason| match reason {
            ...
            ShopPurchaseBlockReason::CardService(reason) => match reason {
                ...
                td_core::CardServicePurchaseBlockReason::NotEnoughUnengravedCards {
                    required,
                    available,
...
impl Component for TooltipLayer {
    fn render(self, ctx: &RenderCtx) {
        ...
        let opened_modals = game_state.opened_modals();
        let modal_open = (opened_modals.user.is_some(), opened_modals.system.is_some());
        ...
        let deck_revision = ctx.track_eq(&game_state.raw_core_state().deck().revision);
        ...
        ctx.compose(|ctx| {
            let sections = request.content.sections(locale, &game_state);
