...
impl MonsterConfig {
    pub(crate) fn to_core_state(&self) -> td_core::MonsterConfigState {
        td_core::MonsterConfigState {
            stats: self
                .stats
                .iter()
    ...
    pub(crate) fn from_core_state(state: td_core::MonsterConfigState) -> Option<Self> {
        let mut stats = BTreeMap::new();
        for entry in state.stats {
            let kind = MonsterKind::from_core_raw(entry.kind)?;
            if stats
                .insert(
                    kind,
                    MonsterStats {
...
mod tests {
    ...
    #[test]
    fn monster_config_raw_state_round_trips_without_host_types() {
        let config = default_monster_config();
        let state = config.to_core_state();
        let restored = MonsterConfig::from_core_state(state).expect("valid monster config");

        assert_eq!(restored, config);
    }

    #[test]
    fn monster_config_raw_state_rejects_duplicate_stats_kinds() {
        let config = default_monster_config();
        let mut state = config.to_core_state();
        state.stats.push(state.stats[0].clone());

        assert!(MonsterConfig::from_core_state(state).is_none());
    }
}
