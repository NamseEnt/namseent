...
use crate::game_state::upgrade::UpgradePresentation;
...
impl GameState {
    ...
    pub(crate) fn preserve_discoveries_from(&mut self, previous: &DiscoveryState) {
        let runtime = self.pending_discoveries.clone();
        ...
        self.pending_discoveries = merged;
    }

    fn discover(&mut self, discovery: Discovery) {
        if self.is_headless() || !self.pending_discoveries.insert(discovery) {
            return;
        }
        if self.pending_discoveries.loaded {
            self.pending_discoveries.dirty = true;
        }
    }
}

impl crate::headed_game::HeadedGame {
    pub(crate) fn merge_pending_discoveries(&mut self, pending: DiscoveryState) {
        let inserted = self.discovery.merge(&pending);
        if inserted && self.discovery.loaded {
    ...
    pub(crate) fn persist_discoveries_if_dirty(&mut self) {
        if let Some(snapshot) = self.discovery.take_dirty_snapshot(self.state.is_headless()) {
...
mod tests {
    ...
    fn loading_merges_runtime_discoveries_and_marks_new_entries_dirty() {
        ...
        game_state
            .pending_discoveries
            .insert(Discovery::Item("lump_sugar".to_string()));
        game_state.pending_discoveries.loaded = true;
        game_state.pending_discoveries.dirty = true;
        assert!(game_state.pending_discoveries.loaded);
        assert!(game_state.pending_discoveries.dirty);
        assert!(!game_state.pending_discoveries.items.is_empty());
    }

    #[test]
    fn headless_game_state_does_not_collect_discoveries() {
        let mut game_state = crate::game_state::create_initial_game_state();
        let before = game_state.pending_discoveries.clone();
        game_state.headless = true;
        game_state.discover(Discovery::Item("lump_sugar".to_string()));
        assert_eq!(game_state.pending_discoveries, before);
    }
}
