...
use crate::game_state::upgrade::{
    Upgrade, UpgradeDiscriminants, UpgradePresentation, UpgradeWithId,
};
...
pub(super) fn all_entries() -> Vec<Entry> {
    ...
    for discriminant in CardServiceDiscriminants::iter() {
        ...
        entries.push(Entry {
            ...
            stable_key: card_service.key().to_string(),
            content: EntryContent::CardService(card_service),
        });
    }
    for discriminant in UpgradeDiscriminants::iter() {
        let treasure =
            UpgradeWithId::from_core_state(td_core::generated_upgrade(discriminant.core_kind()))
                .expect("core catalog upgrade payload must decode")
                .upgrade;
        entries.push(Entry {
            kind: EntryKind::Treasure,
            rarity: discriminant.rarity(),
            key: treasure.key().to_string(),
            stable_key: discriminant.core_kind_key().to_string(),
