...
use namui::*;
pub(crate) use render::Animation;
pub use render::{AnimationKind, tower_animation_tick};
pub(crate) use royal_straight_flush::RoyalStraightFlushVisual;
...
pub(crate) use royal_straight_flush::{
    RoyalStraightFlushVisualParams, spawn_royal_straight_flush_visual,
};
pub use skill::*;
use std::ops::Deref;
...
pub struct Tower {
    ...
    core_damage_multiplier_raw: i64,
...
pub(crate) struct TowerPresentationState {
    animation: Animation,
    royal_straight_flush_visual: Option<RoyalStraightFlushVisual>,
}

impl Tower {
    pub(crate) fn presentation_state(&self) -> TowerPresentationState {
        TowerPresentationState {
            animation: self.animation.clone(),
            royal_straight_flush_visual: self.royal_straight_flush_visual.clone(),
        }
    }

    pub(crate) fn restore_presentation_state(&mut self, state: TowerPresentationState) {
        self.animation = state.animation;
        self.royal_straight_flush_visual = state.royal_straight_flush_visual;
    }

    pub(crate) fn to_core_state(&self) -> td_core::TowerState {
        let engraving_modifier = self.engraving_modifier();
        td_core::TowerState {
            id: self.id.map(|id| id.raw()),
            left_top: [self.left_top.x, self.left_top.y],
            cooldown: self.cooldown.ticks(),
            template: self.template.to_core_state(),
            status_effects: self
                .status_effects
                .iter()
                .map(TowerStatusEffect::to_core_status_effect)
                .collect(),
            skills: self.skills.iter().map(TowerSkill::to_core_skill).collect(),
            damage_multiplier_raw: self.core_damage_multiplier_raw,
            attack_range_radius_raw: self.attack_range_radius().raw(),
            effective_shoot_interval: self.template.effective_shoot_interval().ticks(),
            on_hit_splashes: engraving_modifier
                .on_hit_splashes
                .into_iter()
                .map(crate::card::EngravingSplash::to_core_state)
                .collect(),
            on_attack_splashes: engraving_modifier
                .on_attack_splashes
                .into_iter()
                .map(crate::card::EngravingSplash::to_core_state)
                .collect(),
    ...
    pub(crate) fn from_core_state(state: td_core::TowerState, sim_tick: SimTick) -> Option<Self> {
        let template = TowerTemplate::from_core_state(state.template)?;
        let mut tower = Self::new(
            &template,
            MapCoord::new(state.left_top[0], state.left_top[1]),
            sim_tick,
        );
        tower.cooldown = SimTickSpan::from_ticks(state.cooldown);
        tower.core_damage_multiplier_raw = state.damage_multiplier_raw;
        tower.status_effects = state
            .status_effects
            .into_iter()
        ...
        if let Some(id) = state.id {
            tower.assign_id(TowerId::from_raw(id));
        }
        Some(tower)
    }
}
...
impl Tower {
    pub fn new(template: &TowerTemplate, left_top: MapCoord, sim_tick: SimTick) -> Self {
        Self {
            ...
            skills: template
                ...
                .collect(),
            core_damage_multiplier_raw: FixedRatio::ONE.raw(),
            cached_upgrade: CachedTowerUpgradeDamage {
                revision: 0,
                bonuses: Vec::new(),
    ...
    pub fn refresh_cached_upgrade_damage(
        ...
        self.core_damage_multiplier_raw = self
            .damage_multiplier_raw(&self.cached_upgrade.bonuses)
            .saturating_sub(self.card_polish_pct().raw());
        self.cached_upgrade.revision = revision;
    }

    pub fn refresh_cached_upgrade_damage_raw(&mut self, revision: usize, upgrade_bonus_raw: i64) {
        let mut snapshot = self.to_core_state();
        snapshot.damage_multiplier_raw = FixedRatio::ONE.raw().saturating_add(upgrade_bonus_raw);
        self.cached_upgrade.damage = Damage::from_raw(snapshot.attack_damage_raw());
        self.core_damage_multiplier_raw = snapshot.damage_multiplier_raw;
        self.cached_upgrade.revision = revision;
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn animation_state(&self) -> Animation {
...
impl TowerTemplate {
    ...
    pub(crate) fn damage_multiplier_raw(
        &self,
        tower_upgrade_bonuses: &[crate::game_state::upgrade::TowerUpgradeDamageBonus],
    ) -> i64 {
        let upgrade_bonus_sum = tower_upgrade_bonuses
            ...
            .fold(0_i64, i64::saturating_add)
            .saturating_add(self.card_polish_pct().raw());
        FixedRatio::ONE.raw().saturating_add(upgrade_bonus_sum)
    }
}

impl TowerTemplate {
    pub fn to_core_state(&self) -> td_core::TowerTemplateState {
        td_core::TowerTemplateState {
            kind: self.kind.to_core_raw(),
            rerolled_count: self.rerolled_count,
            shoot_interval: self.shoot_interval.ticks(),
            default_attack_range_radius_raw: self.default_attack_range_radius.raw(),
            default_damage_raw: self.default_damage.raw(),
            suit: self.suit.map(crate::card::suit_to_core_raw),
            rank: self.rank.map(|rank| rank as u8),
            skill_templates: self
                .skill_templates
                .iter()
                .map(|template| {
                    TowerSkill::new(*template, SimTick::ZERO)
                        .to_core_skill()
                        .template
                })
                .collect(),
            default_status_effects: self
    ...
    pub(crate) fn from_core_state(state: td_core::TowerTemplateState) -> Option<Self> {
        ...
        let suit = match state.suit {
            Some(value) => Some(crate::card::suit_from_core_raw(value)?),
            None => None,
        };
        let rank = match state.rank {
            Some(value) => Some(crate::card::rank_from_core_raw(value)?),
            None => None,
        };
        let used_cards = state
            .used_cards
            .into_iter()
            .map(Card::from_core_state)
            .collect::<Option<Vec<_>>>()?;
        let mut template = Self::new_optional_with_used_cards(kind, suit, rank, used_cards);
        template.rerolled_count = state.rerolled_count;
        template.shoot_interval = SimTickSpan::from_ticks(state.shoot_interval);
        template.default_attack_range_radius =
            WorldDistance::from_raw(state.default_attack_range_radius_raw);
        template.default_damage = Damage::from_raw(state.default_damage_raw);
        template.skill_templates = state
            .skill_templates
            .into_iter()
            .map(|template| {
                TowerSkill::from_core_skill(td_core::TowerSkill {
                    last_used_at: 0,
                    template,
                })
                .template
            ...
            .collect();
        template.default_status_effects = state
            .default_status_effects
            .into_iter()
            .map(TowerStatusEffect::from_core_status_effect)
            .collect();
...
const ALL_TOWER_KINDS: [TowerKind; 11] = [
...
impl TowerKind {
    ...
    pub(crate) fn from_core_raw(value: u8) -> Option<Self> {
        ALL_TOWER_KINDS.get(value as usize).copied()
    }
...
mod tests {
    ...
    #[test]
    fn all_tower_kinds_have_stable_lossless_raw_mapping() {
        assert_eq!(ALL_TOWER_KINDS.len(), 11);
        for (index, kind) in ALL_TOWER_KINDS.iter().copied().enumerate() {
            assert_eq!(kind.to_core_raw(), index as u8);
            assert_eq!(TowerKind::from_core_raw(index as u8), Some(kind));
        }
        assert_eq!(TowerKind::from_core_raw(11), None);
    }

    #[test]
    fn tower_template_state_round_trips_cards_skills_and_scalars() {
        let template = TowerTemplate::new_optional_with_used_cards(
            TowerKind::OnePair,
            Some(Suit::Hearts),
            Some(Rank::Queen),
            vec![Card {
                id: CardId::from_raw(21),
                suit: Suit::Hearts,
                rank: Rank::Queen,
                effects: CardEffects {
                    polish_pct: FixedRatio::from_raw(125_000),
                    engraving: Some(Engraving::Overcharge),
                },
            }],
        );
        let restored = TowerTemplate::from_core_state(template.to_core_state())
            .expect("valid tower template state");

        assert_eq!(restored.kind, template.kind);
        assert_eq!(restored.rerolled_count, template.rerolled_count);
        assert_eq!(restored.shoot_interval, template.shoot_interval);
        assert_eq!(restored.default_damage, template.default_damage);
        assert_eq!(restored.suit, template.suit);
        assert_eq!(restored.rank, template.rank);
        assert_eq!(restored.used_cards(), template.used_cards());
        assert_eq!(restored.skill_templates, template.skill_templates);
        assert_eq!(
            restored.default_status_effects,
            template.default_status_effects
        );
        assert_eq!(restored.engraving_modifier(), template.engraving_modifier());
    }

    #[test]
    fn tower_state_round_trips_authoritative_fields_without_presentation() {
        let template = TowerTemplate::new_optional_with_used_cards(
            TowerKind::OnePair,
            Some(Suit::Hearts),
            Some(Rank::Queen),
            vec![Card {
                id: CardId::from_raw(31),
                suit: Suit::Hearts,
                rank: Rank::Queen,
                effects: CardEffects::default(),
            }],
        );
        let mut tower = Tower::new(&template, MapCoord::new(3, 4), SimTick::from_ticks(12));
        tower.assign_id(TowerId::from_raw(8));
        tower.cooldown = SimTickSpan::from_ticks(7);

        let restored = Tower::from_core_state(tower.to_core_state(), SimTick::from_ticks(20))
            .expect("valid tower state");
        assert_eq!(restored.id(), TowerId::from_raw(8));
        assert_eq!(restored.left_top, tower.left_top);
        assert_eq!(restored.cooldown_ticks(), tower.cooldown_ticks());
        assert_eq!(
            restored.template.to_core_state(),
            tower.template.to_core_state()
        );
        assert_eq!(restored.status_effects, tower.status_effects);
        assert_eq!(
            restored
                .skills
                .iter()
        ...
        let mut game_state = crate::game_state::create_game_state_with_seed(23);
        game_state.towers.place_tower(tower.clone());
        let snapshots = game_state.presentation_projection().tower_snapshots();
        let presentation = game_state
            .towers
            .iter()
            .next()
            .unwrap()
            .presentation_state();
        assert_eq!(snapshots, vec![tower.to_core_state()]);
        assert!(
            game_state
                .presentation_projection_mut()
                .restore_tower_snapshots(snapshots, SimTick::from_ticks(20))
        );
        assert_eq!(
            game_state.towers.iter().next().unwrap().id(),
            TowerId::from_raw(8)
        );
        assert!(
            game_state
        ...
        let mut invalid = game_state.presentation_projection().tower_snapshots();
        invalid[0].template.kind = u8::MAX;
        assert!(
            !game_state
                .presentation_projection_mut()
                .restore_tower_snapshots(invalid, SimTick::from_ticks(20))
        );
        assert_eq!(
            game_state.towers.iter().next().unwrap().id(),
            TowerId::from_raw(8)
        ...
        let mut duplicate = game_state.presentation_projection().tower_snapshots();
        duplicate.push(duplicate[0].clone());
        assert!(
            !game_state
                .presentation_projection_mut()
                .restore_tower_snapshots(duplicate, SimTick::from_ticks(20))
        );
        assert_eq!(game_state.towers.iter().count(), 1);
        assert_eq!(
            game_state.towers.iter().next().unwrap().id(),
            TowerId::from_raw(8)
        );
    ...
    #[test]
    fn raw_attack_preserves_upgrade_and_card_polish_multipliers_without_double_application() {
        let template = TowerTemplate::new_optional_with_used_cards(
            TowerKind::OnePair,
            Some(Suit::Hearts),
            Some(Rank::Queen),
            vec![Card {
                id: CardId::from_raw(41),
                suit: Suit::Hearts,
                rank: Rank::Queen,
                effects: CardEffects {
                    polish_pct: FixedRatio::from_raw(250_000),
                    engraving: None,
                },
            }],
        );
        let mut tower = Tower::new(&template, MapCoord::new(0, 0), SimTick::ZERO);
        tower.assign_id(TowerId::from_raw(9));
        tower.status_effects.push(TowerStatusEffect {
            kind: TowerStatusEffectKind::DamageMul {
                mul: FixedRatio::from_integer(2),
            },
            end_at: TowerStatusEffectEnd::NeverEnd,
        });
        let bonuses = [crate::game_state::upgrade::TowerUpgradeDamageBonus {
            target: crate::game_state::upgrade::TowerUpgradeTarget::Global,
            bonus_pct: FixedRatio::ONE,
        }];
        tower.refresh_cached_upgrade_damage(1, &bonuses);

        let raw = tower.to_core_state();

        assert_eq!(raw.attack_damage_raw(), tower.cached_upgrade_damage().raw());
    }

    #[test]
    fn tower_snapshot_restore_preserves_legacy_bincode_bytes() {
        let template = TowerTemplate::new(TowerKind::OnePair, Suit::Hearts, Rank::Queen);
        let tower = Tower::new(&template, MapCoord::new(3, 4), SimTick::from_ticks(12));
        let original = namui::bincode::encode_to_vec(&tower, namui::bincode::config::standard())
            .expect("tower encoding");
        let restored = Tower::from_core_state(tower.to_core_state(), SimTick::from_ticks(12))
            .expect("valid tower snapshot");
        let snapshot = namui::bincode::encode_to_vec(&restored, namui::bincode::config::standard())
            .expect("restored tower encoding");

        assert_eq!(original, snapshot);
    }

    #[test]
    fn tower_snapshot_excludes_presentation_animation_state() {
        let template = TowerTemplate::new(TowerKind::OnePair, Suit::Hearts, Rank::Queen);
        let mut tower = Tower::new(&template, MapCoord::new(3, 4), SimTick::from_ticks(12));
        let authoritative = tower.to_core_state();
        tower.animation.kind = AnimationKind::Attack;
        tower.animation.transited_at = SimTick::from_ticks(30);
        tower.animation.tick_at = SimTick::from_ticks(31);
        tower.animation.y_ratio_offset = 0.5;
        tower.animation.y_ratio_velocity = -0.25;

        assert_eq!(tower.to_core_state(), authoritative);
    }

    #[test]
    fn tower_presentation_state_round_trips_without_core_state() {
        let template = TowerTemplate::new(TowerKind::OnePair, Suit::Hearts, Rank::Queen);
        let mut tower = Tower::new(&template, MapCoord::new(3, 4), SimTick::from_ticks(12));
        tower.animation.kind = AnimationKind::Attack;
        tower.animation.transited_at = SimTick::from_ticks(30);
        ...
        tower.animation.y_ratio_offset = 0.5;
        tower.animation.y_ratio_velocity = -0.25;
        let presentation = tower.presentation_state();

        let mut restored = Tower::new(&template, MapCoord::new(3, 4), SimTick::from_ticks(12));
        restored.restore_presentation_state(presentation.clone());

        assert!(restored.presentation_state() == presentation);
        assert_eq!(restored.to_core_state(), tower.to_core_state());
    }
