...
impl Component for ResultModal {
    fn render(self, ctx: &RenderCtx) {
        ...
        let metrics = game_state.state().metrics();
        let clear_rate = match game_state.raw_core_state().flow() {
            td_core::GameFlowState::Result { clear_rate_raw } => {
                crate::ClearRate::from_ratio(crate::FixedRatio::from_raw(*clear_rate_raw))
            }
                                    ...
                                    table::vertical([
                                        table::fixed_no_clip(24.px(), |wh, ctx| {
                                            ctx.add(StatRow {
                                                ...
                                                value: format!(
                                                    "{}",
                                                    metrics.max_consecutive_perfect_clears
                                                ),
                                                icon_kind: None,
                                            });
                                        }),
                                        table::fixed_no_clip(24.px(), |wh, ctx| {
                                            ctx.add(StatRow {
                                                ...
                                                value: format!("{}", metrics.total_gold_earned),
                                                icon_kind: Some(IconKind::Gold),
                                            });
                                        }),
                                        table::fixed_no_clip(24.px(), |wh, ctx| {
                                            let total_damage = metrics
                                                .tower_damage_stats
                                                .iter()
                                        ...
                                        table::fixed_no_clip(24.px(), |wh, ctx| {
                                            ctx.add(StatRow {
                                                wh,
                                                label: game_state.text().result_modal(
                                                    ResultModalText::CardRerollCountLabel,
                                                ),
                                                value: format!("{}", metrics.total_rerolled_count),
                                                icon_kind: None,
                                            });
                                        }),
                                        table::fixed_no_clip(PADDING, |_wh, _ctx| {}),
                                        table::ratio_no_clip(1, |wh, ctx| {
                                            ctx.add(TowerDamagePanel {
                                                wh,
                                                tower_stats: &metrics.tower_damage_stats,
