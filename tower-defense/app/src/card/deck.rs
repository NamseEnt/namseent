...
impl Deck {
    pub fn to_core_state(&self) -> td_core::DeckState {
        td_core::DeckState {
    ...
    pub fn from_core_state(state: td_core::DeckState) -> Option<Self> {
        Some(Self {
...
mod tests {
    ...
    #[test]
    fn raw_state_round_trip_preserves_card_piles() {
        let first = Card::new(Rank::Ace, Suit::Spades);
        let second = Card::new(Rank::King, Suit::Hearts);
        let mut deck = deck_with_draw_pile(vec![first, second]);
        deck.revision = 4;
        deck.next_card_id = 99;
        deck.discard_pile = vec![Card::new(Rank::Queen, Suit::Clubs)];

        let raw = deck.to_core_state();
        let restored = Deck::from_core_state(raw.clone()).expect("valid deck state");
