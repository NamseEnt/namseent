    ...
    fn apply_korean<'a>(self, builder: &mut TypographyBuilder<'a>) {
        match self {
            ...
            EffectText::Description(effect) => match effect {
                ...
                Effect::Shield { amount } => builder.text(format!("{:.0} 피해를 흡수하는 방어막", amount)),
                Effect::ExtraDice => builder.text("추가 주사위 1회"),
                Effect::EarnGold { amount } => builder.text(format!("{} 골드 획득", amount)),
                Effect::Lottery { amount, probability } => builder.text(format!("{:.0}% 확률로 {} 골드 획득", probability*100.0, amount)),
                ...
                Effect::GrantUpgrade { .. } => builder.text("업그레이드 획득"),
                Effect::GrantItem { .. } => builder.text("아이템 획득"),
                Effect::IncreaseAllTowersDamage { multiplier } => builder.text(format!("모든 타워 공격력 +{:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseAllTowersDamage { multiplier } => builder.text(format!("모든 타워 공격력 -{:.0}%", (1.0-multiplier)*100.0)),
                Effect::IncreaseIncomingDamage { multiplier } => builder.text(format!("받는 피해 +{:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseIncomingDamage { multiplier } => builder.text(format!("받는 피해 -{:.0}%", (1.0-multiplier)*100.0)),
                Effect::IncreaseGoldGain { multiplier } => builder.text(format!("골드 획득량 +{:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseGoldGainPercent { reduction_percentage } => builder.text(format!("골드 획득량 -{:.0}%", reduction_percentage*100.0)),
                Effect::DisableItemAndUpgradePurchases => builder.text("아이템/업그레이드 구매 불가"),
                Effect::DisableItemUse => builder.text("아이템 사용 불가"),
                Effect::IncreaseMaxHandSlots { bonus } => builder.text(format!("카드 선택 슬롯 +{}", bonus)),
                Effect::DecreaseMaxHandSlots { penalty } => builder.text(format!("카드 선택 슬롯 -{}", penalty)),
                Effect::IncreaseMaxRerolls { bonus } => builder.text(format!("리롤 +{}", bonus)),
                Effect::DecreaseMaxRerolls { penalty } => builder.text(format!("리롤 -{}", penalty)),
                Effect::IncreaseEnemyHealthPercent { percentage } => builder.text(format!("적 체력 +{:.0}%", percentage)),
                Effect::DecreaseEnemyHealthPercent { percentage } => builder.text(format!("적 체력 -{:.0}%", percentage)),
                Effect::IncreaseEnemySpeed { multiplier } => builder.text(format!("적 이동속도 +{:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseEnemySpeed { multiplier } => builder.text(format!("적 이동속도 -{:.0}%", (1.0-multiplier)*100.0)),
                Effect::RankTowerDisable { rank } => builder.text(format!("{} 랭크 타워 비활성화", rank)),
                Effect::SuitTowerDisable { suit } => builder.text(format!("{} 수트 타워 비활성화", suit)),
                Effect::AddTowerCardToPlacementHand { tower_kind, suit, rank, count } => builder.text(format!("타워 설치 핸드에 {} {} {} 카드를 {}장 추가", suit, rank, tower_kind.to_text().to_korean(), count)),
                Effect::GainShield { min_amount, max_amount } => builder.text(format!("보호막 {}~{} 획득", min_amount, max_amount)),
                Effect::HealHealth { min_amount, max_amount } => builder.text(format!("체력 {}~{} 회복", min_amount, max_amount)),
                Effect::GainGold { min_amount, max_amount } => builder.text(format!("골드 {}~{} 획득", min_amount, max_amount)),
                _ => builder.text(""),
            },
        };
    }
    ...
    fn apply_english<'a>(self, builder: &mut TypographyBuilder<'a>) {
        match self {
            EffectText::Name(effect) => match effect {
                ...
                Effect::DisableItemAndUpgradePurchases => builder.text("Disable Item/Upgrade Purchases"),
                ...
                Effect::IncreaseEnemyHealthPercent { .. } => builder.text("Increase Enemy Health Percent"),
                Effect::DecreaseEnemyHealthPercent { .. } => builder.text("Decrease Enemy Health Percent"),
            ...
            EffectText::Description(effect) => match effect {
                ...
                Effect::Lottery { amount, probability } => builder.text(format!("{}% chance to get {} gold", probability*100.0, amount)),
                ...
                Effect::IncreaseAllTowersDamage { multiplier } => builder.text(format!("Increase tower damage by {:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseAllTowersDamage { multiplier } => builder.text(format!("Decrease tower damage by {:.0}%", (1.0-multiplier)*100.0)),
                Effect::IncreaseIncomingDamage { multiplier } => builder.text(format!("Increase incoming damage by {:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseIncomingDamage { multiplier } => builder.text(format!("Decrease incoming damage by {:.0}%", (1.0-multiplier)*100.0)),
                Effect::IncreaseGoldGain { multiplier } => builder.text(format!("Increase gold gain by {:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseGoldGainPercent { reduction_percentage } => builder.text(format!("Decrease gold gain by {:.0}%", reduction_percentage*100.0)),
                Effect::DisableItemAndUpgradePurchases => builder.text("Item/Upgrade purchase disabled"),
                Effect::DisableItemUse => builder.text("Item use disabled"),
                Effect::IncreaseMaxHandSlots { bonus } => builder.text(format!("+{} max hand slots", bonus)),
                Effect::DecreaseMaxHandSlots { penalty } => builder.text(format!("-{} max hand slots", penalty)),
                Effect::IncreaseMaxRerolls { bonus } => builder.text(format!("+{} rerolls", bonus)),
                Effect::DecreaseMaxRerolls { penalty } => builder.text(format!("-{} rerolls", penalty)),
                Effect::IncreaseEnemyHealthPercent { percentage } => builder.text(format!("Increase enemy health by {}%", percentage)),
                Effect::DecreaseEnemyHealthPercent { percentage } => builder.text(format!("Decrease enemy health by {}%", percentage)),
                Effect::IncreaseEnemySpeed { multiplier } => builder.text(format!("Increase enemy speed by {:.0}%", (multiplier-1.0)*100.0)),
                Effect::DecreaseEnemySpeed { multiplier } => builder.text(format!("Decrease enemy speed by {:.0}%", (1.0-multiplier)*100.0)),
                Effect::RankTowerDisable { rank } => builder.text(format!("Disable rank {} towers", rank)),
                Effect::SuitTowerDisable { suit } => builder.text(format!("Disable suit {} towers", suit)),
                Effect::AddTowerCardToPlacementHand { tower_kind, suit, rank, count } => builder.text(format!("Add {} {} {} tower cards {}", count, rank, suit, tower_kind.to_text().to_english())),
                Effect::GainShield { min_amount, max_amount } => builder.text(format!("Gain shield {}~{}", min_amount, max_amount)),
                Effect::HealHealth { min_amount, max_amount } => builder.text(format!("Heal {}~{}", min_amount, max_amount)),
                Effect::GainGold { min_amount, max_amount } => builder.text(format!("Gain {}~{} gold", min_amount, max_amount)),
                _ => builder.text("")
            },
        };
    }
