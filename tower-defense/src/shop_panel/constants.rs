...
pub(super) const ACTION_GAP: Px = px(8.0);

#[inline]
pub(super) fn action_area_width() -> Px {
    ACTION_WIDTH + INNER_PADDING * 2.0
}

#[inline]
pub(super) fn action_area_height() -> Px {
    ACTION_HEIGHT * 2.0 + ACTION_GAP + INNER_PADDING * 2.0
}

pub(super) const TOP_OUTSIDE_HEIGHT: Px = px(24.0);

pub(super) const ACTION_HEIGHT: Px = px(48.0);
pub(super) const ACTION_WIDTH: Px = px(144.0);
...
pub(super) fn shop_panel_wh() -> Wh<Px> {
    Wh::new(
        panel_width(),
        TOGGLE_HEIGHT + SHOP_PANEL_HEIGHT + TOP_OUTSIDE_HEIGHT,
    )
}
