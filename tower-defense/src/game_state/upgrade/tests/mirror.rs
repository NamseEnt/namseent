...
fn mirror_duplicates_next_acquired_tower() {
    ...
    assert_eq!(game_state.upgrade_state.consume_pending_mirror_count(), 0);
    assert!(game_state.upgrade_state.upgrades.iter().any(|upgrade| {
        matches!(upgrade, Upgrade::NameTag(..))
            && (upgrade.damage_multiplier().unwrap_or_default() - 2.0).abs() < f32::EPSILON
    }));
...
fn mirror_produces_duplicate_towers_in_placing_flow() {
    ...
    game_state.goto_placing_tower(crate::game_state::tower::TowerTemplate::new(
        crate::game_state::tower::TowerKind::High,
        crate::card::Suit::Spades,
        crate::card::Rank::Ace,
    ));

    assert_eq!(game_state.hand.active_slot_ids().len(), 3);
    assert_eq!(game_state.upgrade_state.consume_pending_mirror_count(), 0);
}
