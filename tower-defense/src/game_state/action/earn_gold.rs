...
pub(super) fn add_to_balance(game_state: &mut GameState, amount: usize) {
    game_state.gold += amount;
    game_state.metrics.total_gold_earned += amount;
}

pub(super) fn play_earn_sound(game_state: &mut GameState, amount: usize) {
    if amount == 0 {
        return;
    }
...
pub(super) fn trigger_upgrades(game_state: &mut GameState, amount: usize) {
    ...
    game_state.handle_upgrade_trigger(UpgradeTriggerEvent::GoldEarned { amount });
}
