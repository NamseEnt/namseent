use super::{TILE_PX_SIZE, Tower, mutate_game_state, upgrade::TowerUpgradeState};
use crate::theme::{
...
pub struct TowerStatusBubble<'a> {
    pub tower: &'a Tower,
    pub tower_upgrades: &'a [TowerUpgradeState],
}

impl Component for TowerStatusBubble<'_> {
    fn render(self, ctx: &RenderCtx) {
        ...
        // Position bubble above tower
        let tower_size = TILE_PX_SIZE.to_xy() * 2.0;
        let bubble_x = tower_size.x * 0.5 - BUBBLE_WIDTH * 0.5;
        let bubble_y = -BUBBLE_HEIGHT - 10.px();

        ctx.translate((bubble_x, bubble_y)).compose(|ctx| {
            ctx.compose(|ctx| {
