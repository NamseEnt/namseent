...
fn card_selection_reroll_penalty_then_bonus_sequence() {
    ...
    assert_eq!(gs.max_dice_chance(), 0, "패널티 3회 후 0 유지");
    for _ in 0..5 {
...
fn card_selection_slots_bonus_and_penalty_accumulation() {
    ...
    assert_eq!(gs.stage_modifiers.get_max_hand_slots_bonus(), 0);
    assert_eq!(gs.stage_modifiers.get_max_hand_slots_penalty(), 0);

    run_effect(&mut gs, &Effect::IncreaseMaxHandSlots { bonus: 2 });
    run_effect(&mut gs, &Effect::IncreaseMaxHandSlots { bonus: 1 });
    assert_eq!(
        gs.stage_modifiers.get_max_hand_slots_bonus(),
        3,
        "+2 +1 누적 보너스 = 3"
    );

    run_effect(&mut gs, &Effect::DecreaseMaxHandSlots { penalty: 1 });
    run_effect(&mut gs, &Effect::DecreaseMaxHandSlots { penalty: 2 });
    assert_eq!(
        gs.stage_modifiers.get_max_hand_slots_penalty(),
        3,
        "-1 -2 누적 패널티 = 3"
    );
}
