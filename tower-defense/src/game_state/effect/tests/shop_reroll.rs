...
fn shop_reroll_increase_effect_applies() {
    ...
    gs.stage_modifiers.apply_max_rerolls_bonus(1); // +1
    assert_eq!(gs.max_dice_chance(), 2, "+1 보너스 적용 후 2가 되어야 함");
}

#[test]
fn shop_reroll_decrease_effect_applies() {
    let mut gs = make_test_state();
    gs.stage_modifiers.apply_max_rerolls_penalty(1); // -1 (saturating)
    assert_eq!(
        gs.max_dice_chance(),
        0,
...
fn shop_reroll_stacking_sequence_penalty_then_bonus() {
    let mut gs = make_test_state();
    for _ in 0..4 {
        gs.stage_modifiers.apply_max_rerolls_penalty(1);
    }
    assert_eq!(gs.max_dice_chance(), 0, "패널티 4회 후 포화로 0");
    for _ in 0..6 {
        gs.stage_modifiers.apply_max_rerolls_bonus(1);
    }
    assert_eq!(
        gs.max_dice_chance(),
        3,
