...
pub(crate) fn list() -> &'static [RiskGeneratorFn] {
    const FNS: &[RiskGeneratorFn] = &[
        |rng, _, _| Effect::DecreaseAllTowersDamage {
            multiplier: rng.gen_range(RISK_DECREASE_TOWER_DAMAGE.0..RISK_DECREASE_TOWER_DAMAGE.1),
        },
        |rng, _, _| Effect::IncreaseIncomingDamage {
            multiplier: rng
                .gen_range(RISK_INCREASE_INCOMING_DAMAGE.0..RISK_INCREASE_INCOMING_DAMAGE.1),
        },
        |rng, _, _| Effect::DecreaseGoldGainPercent {
            reduction_percentage: rng
                .gen_range(RISK_DECREASE_GOLD_GAIN_PERCENT.0..RISK_DECREASE_GOLD_GAIN_PERCENT.1),
        },
