...
impl StrawberryItem {
    ...
    pub fn into_item(self) -> Item {
        Item::Strawberry(self)
    }
}

impl ItemBehavior for StrawberryItem {
    ...
    fn l10n_name<'a>(
        ...
        builder.static_text(match locale.language {
            crate::l10n::Language::Korean => "딸기",
            crate::l10n::Language::English => "Strawberry",
        });
    }
...
pub(super) const DEFINITION: crate::game_state::item::definition::ItemDefinition =
    crate::game_state::item::definition::ItemDefinition::new(generate_strawberry_item, || {
        crate::Rarity::Common
    });

fn generate_strawberry_item(_rng: &mut dyn rand::RngCore) -> Item {
    StrawberryItem::standard().into_item()
}
...
mod tests {
    ...
    #[test]
    fn generator_returns_standard_strawberry() {
        let mut rng = StdRng::seed_from_u64(1);

        assert_eq!(
            generate_strawberry_item(&mut rng),
            StrawberryItem::standard().into_item()
        );
    }

    #[test]
    fn strawberry_has_common_rarity() {
        assert_eq!(
            StrawberryItem::standard().into_item().discriminant(),
            ItemDiscriminants::Strawberry
        );
        assert_eq!(
            ItemDiscriminants::Strawberry.rarity(),
            crate::Rarity::Common
        );
    }

    #[test]
    fn using_strawberry_increases_max_health_and_heals_three_health() {
        let mut game_state = crate::game_state::create_initial_game_state();
        let base_max_hp = game_state.max_hp();
        game_state.hp = base_max_hp - 5.0;

        StrawberryItem::standard().use_item(&mut game_state);

        assert_eq!(game_state.max_hp(), base_max_hp + 2.0);
        assert_eq!(game_state.hp, base_max_hp - 2.0);
    }
}
