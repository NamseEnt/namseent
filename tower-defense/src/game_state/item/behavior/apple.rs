...
impl ItemBehavior for AppleItem {
    ...
    fn l10n_name<'a>(
        ...
        builder.static_text(match locale.language {
            crate::l10n::Language::Korean => "사과",
            crate::l10n::Language::English => "Apple",
        });
    }
...
pub(super) const DEFINITION: crate::game_state::item::definition::ItemDefinition =
    crate::game_state::item::definition::ItemDefinition::new(generate_apple_item, || {
        crate::Rarity::Common
    });

fn generate_apple_item(_rng: &mut dyn rand::RngCore) -> Item {
    AppleItem::standard().into_item()
}
...
mod tests {
    ...
    #[test]
    fn generator_returns_standard_apple() {
        let mut rng = StdRng::seed_from_u64(1);

        assert_eq!(
            generate_apple_item(&mut rng),
            AppleItem::standard().into_item()
        );
    }

    #[test]
    fn apple_has_common_rarity() {
        assert_eq!(
            AppleItem::standard().into_item().discriminant(),
            ItemDiscriminants::Apple
        );
        assert_eq!(ItemDiscriminants::Apple.rarity(), crate::Rarity::Common);
    }

    #[test]
    fn using_apple_increases_max_health_and_heals_six_health() {
        ...
        AppleItem::standard().use_item(&mut game_state);

        assert_eq!(game_state.max_hp(), base_max_hp + 4.0);
