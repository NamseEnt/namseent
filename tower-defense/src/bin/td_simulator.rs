...
use tower_defense::simulator::environment::{AgentAction, LegalAction, Observation};
use tower_defense::simulator::hp_balance::{self, BalanceOptions};
use tower_defense::simulator::ml::MlContract;
use tower_defense::simulator::ml::cli::{self, Command as MlCommand};
use tower_defense::simulator::ml::features::observation_features;
use tower_defense::simulator::ml::model::{
    DeepSetsActorCritic, InferenceBackend, PolicyDevice, default_policy_device,
};
use tower_defense::simulator::ml::neural_checkpoint::NeuralCheckpoint;
use tower_defense::simulator::ml::rollout::logits_for;
use tower_defense::simulator::ml::seed::SeedRange;
use tower_defense::simulator::ml::validation::{
    BaselinePolicy, evaluate_baseline, evaluate_checkpoint, paired_baseline_report,
};
use tower_defense::simulator::policy_runner::{BatchResult, PolicyRunnerConfig, run_batch};
use tower_defense::simulator::recording::{SimRecorder, SimulationProvenance};
...
fn run_simulate(options: SimulateOptions) -> Result<()> {
    ...
    let runner_config = PolicyRunnerConfig {
        ...
        max_stage: None,
        reward_config: tower_defense::simulator::environment::RewardConfig::default(),
    };
...
fn choose_model_action(
    ...
    let candidate_rows = legal_actions
        .iter()
        .map(|legal| {
            tower_defense::simulator::ml::encoding::candidate_entity_rows(
                observation,
                &legal.action,
            )
            .pop()
