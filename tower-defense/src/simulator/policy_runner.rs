...
use crate::deterministic_rng::{domain, rng_for, uniform_index};
...
pub fn run_scripted_oracle_trajectory(
    game_config: Arc<GameConfig>,
    seed: u64,
) -> Result<crate::simulator::trajectory::Trajectory> {
    let episode = run_episode(
    ...
    let steps = episode
        ...
        .as_ref()
        .expect("scripted oracle records steps");
    let mut trajectory = crate::simulator::trajectory::Trajectory::from_policy_steps(
        game_config.as_ref(),
        seed,
        steps,
    );
    trajectory.set_outcome(crate::simulator::trajectory::TrajectoryOutcome {
...
fn monte_carlo_expert_action(
    ...
    for _ in 0..sample_count {
        ...
        let coverage =
            crate::simulator::ml::features::placement_coverage(observation, left, top, &tower.kind);
...
fn card_dps_proxy(cards: &[crate::simulator::environment::CardObservation]) -> f32 {
