...
use crate::simulator::environment::{ActionKind, AgentAction, DecisionPoint, Observation};
...
pub fn observation_features(observation: &Observation) -> Vec<f32> {
    ...
    let card_items = observation
        ...
        .filter_map(|item| match &item.item {
            crate::simulator::environment::HandItemObservation::Card(card) => Some(card),
            crate::simulator::environment::HandItemObservation::Tower(_) => None,
        })
        .collect::<Vec<_>>();
    ...
    match action {
            ...
            let cards = selected_slot_indices.iter().filter_map(|index| {
                observation
                    ...
                    .and_then(|item| match &item.item {
                        crate::simulator::environment::HandItemObservation::Card(card) => {
                            Some(card)
                        }
                        _ => None,
                    })
        ...
        AgentAction::PlaceTower {
            ...
            if let Some(item) = observation.hand.get(*hand_slot_index)
                && let crate::simulator::environment::HandItemObservation::Tower(tower) = &item.item
            {
...
pub fn action_features(action: &AgentAction) -> Vec<f32> {
    candidate_features(
        &Observation {
            ...
            hand: vec![],
            deck: crate::simulator::environment::DeckObservation {
                all_cards: vec![],
                draw_cards: vec![],
                discard_cards: vec![],
            },
            ...
            card_service: None,
            stage_modifiers: crate::simulator::environment::StageModifiersObservation {
