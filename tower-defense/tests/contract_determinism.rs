...
use tower_defense::{Rarity, generate_contract_with_rng};
...
fn contract_generation_is_deterministic_for_same_seed() {
    ...
    assert_eq!(
        format!("{:?}", c1.risk),
        format!("{:?}", c2.risk),
        "risk effect mismatch"
    );
    assert_eq!(
        format!("{:?}", c1.reward),
        format!("{:?}", c2.reward),
        "reward effect mismatch"
    );
}
...
fn contract_generation_varies_for_different_seeds() {
    ...
    assert!(
        !(same_risk && same_reward),
        "Both risk and reward identical for different seeds (rare but indicates insufficient entropy)"
    );
}
