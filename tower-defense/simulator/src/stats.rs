...
pub fn upgrade_rarity_prefix(name: &str) -> Option<&'static str> {
    let kind = td_core::UpgradeKind::ALL
        .iter()
        .find(|kind| kind.key() == name)?;
    Some(td_core::upgrade_rarity_prefix_for_kind(*kind))
}
...
mod tests {
    ...
    use crate::recording::SimRecorder;
    use std::fs;
    ...
    fn summary_tables_cover_every_variant() {
        ...
        let db = Database::open(&db_path).unwrap();
        assert_eq!(db.list_items().unwrap().len(), td_core::ItemKind::COUNT);
        assert_eq!(
    ...
    fn episode_metrics_match_sqlite_rows_and_aggregate_exactly() {
        ...
        use crate::policy_runner::{PolicyRunnerConfig, run_episode};
        use std::sync::Arc;
        ...
        let episode = run_episode(
            ...
            7u64,
            &runner_config,
            |_observation: &crate::environment::Observation,
             legal_actions: &[crate::environment::LegalAction]| {
                legal_actions
                    .first()
        ...
        recorder
            .record_simulation_start_with_provenance(
                ...
                "environment",
                episode.seed,
                &crate::recording::SimulationProvenance {
