...
#[cfg(feature = "simulator-wgpu")]
use super::rollout::{StreamRolloutOptions, stream_rollout_chunks};
...
mod wgpu_learner_tests {
    use super::*;
    use crate::ml::encoding::{EntityRow, TypedObservation};
    use crate::ml::model::{CpuInferenceBackend, DeepSetsActorCritic};
    use crate::ml::rollout::RolloutStep;
    ...
    for local_iteration in 0..config.iterations {
            ...
            .wgpu_stream_rollout
        {
            let (receiver, producer) = stream_rollout_chunks(
                ...
                train_seeds.clone(),
                rollout_config,
                StreamRolloutOptions {
                    chunk_size: config.minibatch_size,
                    max_queued_steps: config.max_queued_steps.max(config.rollout_step_budget),
                    progress: progress.map(|progress| progress.rollout_bar().clone()),
                },
            );
...
mod tests {
    ...
    fn curriculum_resume_preserves_level_counter_and_next_update_logits() {
        ...
        {
            let store = crate::ml::trainer_checkpoint::TrainerCheckpointStore::new(&root).unwrap();
