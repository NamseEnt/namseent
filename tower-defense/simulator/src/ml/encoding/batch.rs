...
use crate::environment::{ActionKind, AgentAction, LegalAction, Observation};

fn tower_identity(tower: &crate::environment::TowerTemplateObservation) -> ([u32; 3], [f32; 2]) {
    (
        [
            ...
            tower
                .suit
                .as_deref()
                .map(crate::ml::vocabulary::suit_id)
                .unwrap_or(0) as u32,
            tower
                .rank
                .as_deref()
                .map(crate::ml::vocabulary::rank_id)
                .unwrap_or(0) as u32,
        ],
        [
...
) -> Option<&crate::environment::TowerTemplateObservation> {
...
pub fn candidate_entity_rows(observation: &Observation, action: &AgentAction) -> Vec<EntityRow> {
    ...
    match action {
        ...
        | AgentAction::DeselectHandCard { hand_slot_index } => {
            ...
            {
                categorical[1] = crate::ml::vocabulary::suit_id(&card.suit) as u32;
                categorical[2] = crate::ml::vocabulary::rank_id(&card.rank) as u32;
                categorical[3] = card.engraving.as_deref().map_or(0, |value| {
                    crate::ml::vocabulary::engraving_key_id(value) as u32
                });
...
mod tests {
    use super::*;
    use crate::ml::encoding::entity::EntityRow;
    ...
    fn policy_batch_keeps_one_legal_candidate_per_row() {
        let environment = crate::environment::GameEnvironment::new(
    ...
    fn placement_candidate_encoding_includes_route_coverage() {
        ...
        let tower = crate::environment::TowerTemplateObservation {
            kind: "high".to_owned(),
            kind_id: 1,
            suit: None,
            rank: None,
