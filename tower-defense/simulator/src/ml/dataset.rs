...
pub fn validate_dataset(dataset: &ExpertDataset, config: &GameConfig) -> Result<()> {
    ...
    let mut contract = dataset.metadata.contract.clone();
    contract
...
pub fn validate_strict_expert_dataset(dataset: &ExpertDataset) -> Result<()> {
    ...
    for episode in &dataset.episodes {
        ...
        if matches!(
            ...
            crate::environment::StepReason::MaxTicks
                | crate::environment::StepReason::MaxDecisions
                | crate::environment::StepReason::NoProgressCycle
        ) {
...
pub fn read_jsonl(path: &Path) -> Result<ExpertDataset> {
    ...
    let mut metadata = metadata.context("expert dataset has no metadata record")?;
    metadata
        .contract
        .migrate_legacy_metadata()
        .map_err(|error| anyhow::anyhow!("{error}"))?;
...
mod tests {
    ...
    fn seed_zero_behavior_dataset_round_trips_with_real_episode() {
        ...
        let outcome = dataset.episodes[0]
            .outcome
            .as_ref()
            .expect("behavior episode has an outcome");
