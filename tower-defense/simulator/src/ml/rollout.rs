...
#[cfg(feature = "simulator-wgpu")]
#[allow(dead_code)]
pub(crate) struct StreamRolloutOptions {
    pub(crate) chunk_size: usize,
    pub(crate) max_queued_steps: usize,
    pub(crate) progress: Option<ProgressBar>,
}

#[cfg(feature = "simulator-wgpu")]
type StreamRolloutMessage = (usize, RolloutBatch, Arc<StepGate>, usize);

#[cfg(feature = "simulator-wgpu")]
type StreamRolloutReceiver = Receiver<Result<StreamRolloutMessage>>;
...
pub(crate) fn stream_rollout_chunks<B: Backend + Send + Sync + 'static>(
    ...
    seeds: Vec<u64>,
    rollout_config: RolloutConfig,
    options: StreamRolloutOptions,
) -> (StreamRolloutReceiver, JoinHandle<()>)
where
    B::Device: Send + Sync + 'static,
{
    let (sender, receiver) = sync_channel(2);
    let step_gate = Arc::new(StepGate::new(options.max_queued_steps));
    ...
    let handle = std::thread::spawn(move || {
        let result = (|| -> Result<()> {
            rollout_config
                ...
                .validate_gamma(rollout_config.gamma)
                .map_err(anyhow::Error::msg)?;
            let episode_chunk_size = (options.chunk_size / 256).clamp(1, 32);
...
fn typed_batches(
    observation: &Observation,
) -> [PaddedEntityBatch; crate::ml::encoding::ENTITY_SET_COUNT] {
...
mod tests {
    ...
    use super::*;
    use crate::config::GameConfig;
    use crate::ml::model::default_policy_device;
    ...
    fn release_throughput_smoke_benchmark() {
        let device = crate::ml::model::default_policy_device();
        let model =
            crate::ml::model::DeepSetsActorCritic::<crate::ml::model::InferenceBackend>::new(
                crate::ml::model::ModelConfig::default(),
                &device,
            );
