...
mod tests {
    ...
    use crate::ml::encoding::{EntityRow, EntitySet};
    use crate::ml::features::{candidate_features, observation_features};
    use std::sync::Arc;
    ...
    fn typed_forward_has_expected_shapes_and_responds_to_entity_identity() {
        ...
        let environment = GameEnvironment::new(config, 7);
        let observation = environment.snapshot();
        let typed = crate::ml::encoding::TypedObservation::from_observation(&observation);
        let batches = std::array::from_fn(|index| {
            crate::ml::encoding::PaddedEntityBatch::from_sets(&[typed.sets[index].clone()])
        });
    ...
    fn candidate_context_is_isolated_per_transition_group() {
        ...
        let first = crate::ml::encoding::EntityRow::new([1, 1, 0, 0], vec![1.0; 5]);
        let second = crate::ml::encoding::EntityRow::new([1, 2, 0, 0], vec![2.0; 5]);
        let candidates = PaddedEntityBatch::from_sets(&[
            crate::ml::encoding::EntitySet::new(vec![first.clone()]),
            crate::ml::encoding::EntitySet::new(vec![second.clone()]),
            crate::ml::encoding::EntitySet::new(vec![first.clone()]),
            crate::ml::encoding::EntitySet::new(vec![second.clone()]),
        ]);
        ...
        let changed_candidates = PaddedEntityBatch::from_sets(&[
            ...
            crate::ml::encoding::EntitySet::new(vec![second.clone()]),
            crate::ml::encoding::EntitySet::new(vec![first.clone()]),
            crate::ml::encoding::EntitySet::new(vec![first]),
        ]);
    ...
    fn typed_candidate_forward_has_finite_logits() {
        ...
        let candidates = crate::ml::encoding::PolicyBatch::from_observation(
            &observation,
            &environment.legal_actions(),
        );
        let typed_state = model.encode_typed_sets(&state_sets, &device);
        let candidate_set = crate::ml::encoding::PaddedEntityBatch::from_sets(
            &candidates
                .candidate_rows
                .iter()
                .map(|rows| crate::ml::encoding::EntitySet::new(rows.clone()))
                .collect::<Vec<_>>(),
        );
