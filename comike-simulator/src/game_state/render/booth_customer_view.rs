...
impl Component for &BoothCustomerView {
    fn render(self, ctx: &RenderCtx) {
        let BoothCustomerView {
        ...
        ctx.compose(|ctx| {
            ctx.translate(Xy::new(20.px(), 404.px()))
                .add(GridStorageBox {
                    on_click_cell: &|cell_xy| {
                        mutate_game_state(move |game_state| {
                            game_state.open_grid_storage_cell_popup(cell_xy);
                        });
                    },
                });
        });
...
struct GridStorageBox<'a> {
    pub on_click_cell: &'a dyn Fn(Xy<usize>),
}

impl Component for GridStorageBox<'_> {
    fn render(self, ctx: &RenderCtx) {
        let Self { on_click_cell } = self;
