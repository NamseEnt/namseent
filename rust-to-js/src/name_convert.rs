...
pub fn def_normalized_name<'tcx>(
    tcx: TyCtxt<'tcx>,
...
pub fn generic_args_name<'tcx>(
    ...
    generic_args
        ...
        .filter_map(|arg| match arg.kind() {
            ...
            rustc_type_ir::GenericArgKind::Type(ty) => Some(ty_name(tcx, &ty)),
            rustc_type_ir::GenericArgKind::Const(const_) => Some(const_name(tcx, const_)),
        })
        .collect::<Vec<_>>()
        .join("__")
}

pub fn const_name<'tcx>(_tcx: TyCtxt<'tcx>, const_: rustc_middle::ty::Const<'tcx>) -> String {
    match const_.kind() {
...
pub fn ty_name<'tcx>(tcx: TyCtxt<'tcx>, ty: &Ty<'tcx>) -> String {
    match ty.kind() {
        ...
        Adt(adt_def, generic_args) => def_normalized_name(tcx, &adt_def.did(), generic_args),
        Foreign(_) => todo!(),
        Str => "_ty_str".to_string(),
        Array(ty, const_) => {
            format!("_ty_array_{}_{}", ty_name(tcx, ty), const_)
        }
        Pat(_, _) => todo!(),
        Slice(ty) => {
            format!("_ty_slice_{}", ty_name(tcx, ty))
        }
        RawPtr(_, _mutability) => todo!(),
        Ref(_region, ty, _mutability) => {
            format!("_ref_{}", ty_name(tcx, ty))
        }
        FnDef(id, args) => def_normalized_name(tcx, id, args),
        ...
        Closure(def_id, generic_args) => def_normalized_name(tcx, def_id, generic_args),
        ...
        Tuple(tuple) => {
            ...
            for ty in tuple.iter() {
                s.push_str(&format!("_{}", ty_name(tcx, &ty)));
            }
            s
        }
