use crate::engine::{self, get_text_width, managers, render::Selection, TextInput};
...
pub(crate) fn get_selection_on_click(
    ...
    let is_shift_key_pressed = managers
        .keyboard_manager
        .any_code_press([engine::Code::ShiftLeft, engine::Code::ShiftRight].to_vec());
    ...
    let aligned_x = match text_input.text_align {
        engine::TextAlign::Left => click_x,
        engine::TextAlign::Center => click_x - (text_input.width / 2.0 - text_width / 2.0),
        engine::TextAlign::Right => text_width - (text_input.width - click_x),
    };
...
fn get_selection_index_of_x(font: &engine::Font, text: &str, x: f32) -> usize {
