use crate::{Config, Error, context, namsh, queue, stack_hash};
...
impl Handler {
    fn process_dump(&self, dump_path: &Path) -> Result<(), Error> {
        let stack_hash_hex = stack_hash::compute(dump_path)?;
        let session_uptime_sec = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .map(|d| d.as_secs().saturating_sub(self.parent_start_unix))
            .unwrap_or(0);
        let ctx = context::collect(&self.config, &self.install_id, session_uptime_sec);
        queue::write_sidecar(
            dump_path,
            &queue::PendingEntry {
                stack_hash: stack_hash_hex.clone(),
                context: ctx,
            },
        )?;
