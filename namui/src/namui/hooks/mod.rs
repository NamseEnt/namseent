...
pub(crate) fn on_raw_event(event: RawEvent) {
    if let Some(ctx) = TREE_CTX.get() { ctx.on_raw_event(event) }
}

pub(crate) fn render_and_draw() {
    if let Some(ctx) = TREE_CTX.get() { ctx.render_and_draw() }
}

pub(crate) fn stop_event_propagation() {
    if let Some(ctx) = TREE_CTX.get() { ctx.stop_event_propagation() }
}
