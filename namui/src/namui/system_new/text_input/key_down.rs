...
fn get_paragraph(props: &text_input::TextInput) -> Option<Paragraph> {
    let font = crate::font::get_font(props.font)?;
...
fn get_selection_on_keyboard_down(
    ...
    let Some(paragraph) = get_paragraph(props) else {
        return Selection::None;
    };

    let next_selection_end = get_caret_index_after_apply_key_movement(key, paragraph, &range);
...
fn get_caret_index_after_apply_key_movement(
    ...
    let caret = paragraph.into_caret(selection.end);
