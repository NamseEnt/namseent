...
fn get_one_click_selection(
    ...
    let selection_index_of_xy =
        get_selection_index_of_xy(text_input_props, fonts, paragraph, click_local_xy, paint);
...
fn get_selection_index_of_xy(
    ...
    let line_len = paragraph.line_len();
    if line_len == 0 {
        return 0;
    }
    ...
    let str_index_before_line = paragraph.char_index_before_line(line_index);

    let line = paragraph.iter_str().nth(line_index).unwrap();

    let glyph_widths = get_text_widths(&line, &fonts, paint);
