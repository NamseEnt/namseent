...
async fn load_fallback_font_typefaces() -> Result<()> {
...
async fn get_noto_color_emoji_typeface() -> Result<Typeface> {
...
pub async fn load_sans_typeface_of_all_languages() -> Result<()> {
...
async fn get_typeface_file_urls() -> Result<TypefaceFileUrls> {
...
async fn get_typeface_files(
...
) -> Result<HashMap<TypefaceType, Vec<u8>>> {
    let iter = try_join_all(typeface_file_urls.iter().map(
        |(typeface_type, font_file_url)| async move {
            ...
            let result: Result<_> = match get_file_from_bundle_with_cached(&url).await {
