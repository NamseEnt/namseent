...
fn auto_scroll_view_with_fit_timing_issue() {
            ...
            ctx.add(AutoScrollViewWithCtx {
                ...
                content: move |ctx| {
                    ...
                    let content = ctx.ghost_compose("dynamic_content", |ctx| {
                        vertical([fit(FitAlign::LeftTop, move |compose_ctx| {
                            // 동적으로 변경되는 텍스트 콘텐츠
                            println!(
                                "  fit render called with height: {:?}",
                                dynamic_content_height
                            );
                    ...
                    scroll_bounding_box_tracker
                        .lock()
                        .unwrap()
                        .push(bounding_box.map(|bb| bb.height()));

                    // 디버깅: rendering_tree의 내용 확인
                    println!(
                        "Frame: content_height={:?}, calculated_bounding_box={:?}",
    ...
    if scroll_bounding_boxes.len() >= 4 {
        ...
        println!(
            "Frame 3 (content=200px, but should show 50px): {:?}",
            scroll_bounding_boxes[2]
        );
        ...
        if let Some(Some(frame3_height)) = scroll_bounding_boxes.get(2) {
            println!(
                "Frame 3 analysis: content is 200px, but bounding_box shows {}px",
                frame3_height
            );
            // 이것이 1프레임 지연의 증거
        }
        ...
        if let Some(Some(frame4_height)) = scroll_bounding_boxes.get(3) {
            assert!(
                *frame4_height >= 150.px(),
                "Frame 4 should finally show correct 200px content, got {:?}",
                frame4_height
            );
        }
    }
...
fn fit_first_frame_rendering_behavior() {
    ...
    let first_frame_calls: Vec<_> = render_calls
        .iter()
