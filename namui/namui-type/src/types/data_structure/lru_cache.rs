...
pub struct LruCache<Key: Hash + Eq, Value, const CAPACITY: usize = 1024> {
    map: OnceLock<Mutex<lru::LruCache<Key, Arc<Value>>>>,
}

impl<Key: Hash + Eq, Value, const CAPACITY: usize> LruCache<Key, Value, CAPACITY> {
    ...
    pub fn get_or_create(&self, key: Key, create: impl FnOnce(&Key) -> Value) -> Arc<Value> {
        ...
        match map.get(&key) {
            ...
            None => {
                ...
                map.get_or_insert(key, || value).clone()
            }
        }
    }
    ...
    pub fn get_or_try_create(
        ...
        map.get(&key).cloned().or_else(|| {
            ...
            Some(map.get_or_insert(key, || Arc::new(value)).clone())
        })
    }
...
impl<Key: Hash + Eq, Value, const CAPACITY: usize> Default for LruCache<Key, Value, CAPACITY> {
    fn default() -> Self {
        Self::new()
    }
}
