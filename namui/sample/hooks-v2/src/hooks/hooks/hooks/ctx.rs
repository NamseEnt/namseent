...
static mut CTX_STACK: OnceLock<Vec<RenderCtx>> = OnceLock::new();
...
pub(crate) fn set_up_before_render(
    ...
    ctx_stack.push(RenderCtx::new(context_for, component_instance));
}

pub(crate) fn take_ctx_and_clear_up() -> RenderCtx {
    let ctx_stack = unsafe { CTX_STACK.get_mut() }.unwrap();
...
pub(crate) fn ctx() -> &'static RenderCtx {
