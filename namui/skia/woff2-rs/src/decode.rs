...
mod tests {
    ...
    use crate::test_resources::{
        FONTAWESOME_REGULAR_400, LATO_V22_LATIN_REGULAR, NOTO_SANS_KR_BLACK, NOTO_SANS_KR_MEDIUM,
    };
    ...
    #[test]
    fn read_noto_sans_black() {
        let buffer = NOTO_SANS_KR_BLACK;
        let ttf = convert_woff2_to_ttf(&mut Cursor::new(buffer)).unwrap();
        assert_eq!(None, ttf_parser::fonts_in_collection(&ttf));
        let _parsed_ttf = ttf_parser::Face::from_slice(&ttf, 1).unwrap();
    }
    #[test]
    fn read_noto_sans_medium() {
        let buffer = NOTO_SANS_KR_MEDIUM;
        let ttf = convert_woff2_to_ttf(&mut Cursor::new(buffer)).unwrap();
        assert_eq!(None, ttf_parser::fonts_in_collection(&ttf));
        let _parsed_ttf = ttf_parser::Face::from_slice(&ttf, 1).unwrap();
    }
