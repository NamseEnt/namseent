...
impl BoundingBox for &PathDrawCommand {
    fn bounding_box(self) -> Option<Rect<Px>> {
        NativeCalculate::path_bounding_box(&self.path, Some(&self.paint))
    }
}
impl BoundingBox for &TextDrawCommand {
    fn bounding_box(self) -> Option<Rect<Px>> {
        ...
        let group_glyph = NativeCalculate::group_glyph(&self.font, &self.paint);
...
impl BoundingBox for &ImageDrawCommand {
    fn bounding_box(self) -> Option<Rect<Px>> {
        match &self.paint {
            Some(paint) => {
                NativeCalculate::path_bounding_box(&Path::new().add_rect(self.rect), Some(paint))
            }
            _ => Some(self.rect),
        }
    }
