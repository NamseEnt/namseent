...
export class OpenFile extends Fd {
    ...
    fd_read(buffer: Uint8Array):
        | {
              ret: typeof wasi.ERRNO_SUCCESS;
              readCount: number;
          }
        | {
              ret: typeof wasi.ERRNO_NOTSUP | typeof wasi.ERRNO_BADF;
          } {
        new Uint8Array(this.file.data, Number(this.file_pos)).set(buffer);
        const readCount = Math.min(
            buffer.length,
            this.file.data.length - Number(this.file_pos),
        );

        this.file_pos += BigInt(readCount);
        return { ret: wasi.ERRNO_SUCCESS, readCount };
    }
...
export class OpenDirectory extends Fd {
    ...
    path_link(path_str: string, inode: Inode, allow_dir: boolean): number {
        ...
        if (entry != null) {
            const source_is_dir =
                inode.stat().filetype == wasi.FILETYPE_DIRECTORY;
            const target_is_dir =
                entry.stat().filetype == wasi.FILETYPE_DIRECTORY;
...
export class Directory extends Inode {
    ...
    get_parent_dir_and_entry_for_path(
        ...
        const entry: Inode | undefined | null =
