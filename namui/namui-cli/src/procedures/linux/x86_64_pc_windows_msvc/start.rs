...
pub async fn start(manifest_path: &Path, release: bool) -> Result<()> {
    ...
    generate_runtime_project(runtime_project::GenerateRuntimeProjectArgs {
    ...
    build_status_service
        .build_started(build_status_service::BuildStatusCategory::Namui)
        .await;
    ...
    let bundle_manifest = bundle::NamuiBundleManifest::parse(project_root_path.clone())?;
...
enum PacketToExtension {
    Build,
    Exit,
}

enum PacketFromExtension {
    Exit,
}
