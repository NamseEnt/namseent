...
impl ImageDrawCommand {
    pub fn from_fit(image: Image, rect: Rect<Px>, fit: ImageFit, paint: Option<Paint>) -> Self {
        let image_wh = image.info().wh();
        let sprite = fit_to_sprite(fit, image_wh, rect);
        Self {
            image,
            sprites: vec![sprite],
            paint,
...
pub fn fit_to_sprite(fit: ImageFit, image_size: Wh<Px>, rect: Rect<Px>) -> ImageSprite {
    let (src_rect, dest_rect) = get_src_dest_rects_in_fit(fit, image_size, rect);
    let scale = if src_rect.width() > 0.px() {
