...
#[derive(Debug, Clone)]
pub struct StaticSetState<State: 'static> {
    sig_id: SigId,
    set_state_tx: std::sync::mpsc::Sender<SetStateItem>,
    _state: std::marker::PhantomData<State>,
}
unsafe impl<State: 'static> Send for StaticSetState<State> {}
unsafe impl<State: 'static> Sync for StaticSetState<State> {}

impl<State: 'static> StaticSetState<State> {
    pub fn set(&self, state: State) {
        self.set_state_tx
            .send(SetStateItem::Set {
                sig_id: self.sig_id,
...
pub(crate) type SendSyncMutateFnOnce =
    Box<dyn FnOnce(&mut (dyn Value + Send + Sync)) + Send + Sync>;
