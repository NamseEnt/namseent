...
class Game {
    ...
    // 비누칠 관련 변수
    private currentSoapingPart: SoapingPart = SoapingPart.HEAD;
    private soapingGrid: boolean[][] = [];
    private gridSize: number = 20;
    private isRubbing: boolean = false;
    private lastRubPosition: {x: number, y: number} | null = null;
    private soapingProgress: Map<SoapingPart, number> = new Map();
    private bubbles: Array<{x: number, y: number, size: number, age: number}> = [];
    ...
    private handleMouseDown(e: MouseEvent): void {
        ...
        if (this.gameState === GameState.SLEEPING) {
            ...
            if (mouseX >= blanketLeft && mouseX <= blanketLeft + blanketWidth &&
                mouseY >= blanketTop && mouseY <= blanketTop + blanketHeight) {
        ...
        // 온도 조절 중 버튼 클릭
            ...
            // 왼쪽 버튼 (차가움)
            if (mouseX >= faucetX - 200 && mouseX <= faucetX - 140 &&
                mouseY >= faucetY - 30 && mouseY <= faucetY + 30) {
                // 랜덤한 만큼 이동 (삐걱거림 효과)
            ...
            // 오른쪽 버튼 (뜨거움)
            if (mouseX >= faucetX + 140 && mouseX <= faucetX + 200 &&
                mouseY >= faucetY - 30 && mouseY <= faucetY + 30) {
                // 랜덤한 만큼 이동 (삐걱거림 효과)
                const randomMove = Math.random() * 15 + 5; // 5-20 만큼 이동
    ...
    private update(deltaTime: number): void {
        ...
        if (this.gameState === GameState.ADJUSTING_TEMPERATURE) {
            ...
            if (this.waterTemperature < 20) {
                ...
                if (this.temperatureTimer > 3000) { // 3초 유지시 성공
    ...
    private nextSoapingPart(): void {
        ...
        if (currentIndex < parts.length - 1) {
            this.currentSoapingPart = parts[currentIndex + 1] as SoapingPart;
            this.initSoapingGrid();
        } else {
            console.log('비누칠 완료!');
            // 다음 단계로
    ...
    private drawTemperatureControl(): void {
        ...
        // 게이지 채우기
        const bodyTempColor = this.bodyTemperature < 30 ? '#4169E1' : 
                             this.bodyTemperature > 70 ? '#DC143C' : '#32CD32';
