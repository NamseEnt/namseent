...
fn render_resize_handles<OnResize: Fn(Circumscribed<Percent>) + 'static>(
    ...
    let on_resize = Arc::new(props.on_resize);
    RenderingTree::Children(
        HANDLES
            .into_iter()
            .map(|handle| {
                let handle_xy = handle.xy(props.rect);
                ...
                .attach_event(move |builder| match props.dragging_context {
                    Some(context) => {
                        ...
                        builder.on_mouse_move_in(on_mouse_move);
                        ...
                        builder.on_mouse_up_in((on_mouse_up)(on_resize.clone()));
