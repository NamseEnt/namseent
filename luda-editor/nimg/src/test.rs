...
fn lossless_rgba_test() {
    ...
    let encoded = encode(ColorFormat::Rgba8888, true, 2, 1, &rgba).unwrap();

    let decoded = decode(&encoded).unwrap();

    assert_eq!(decoded.width, 2);
    assert_eq!(decoded.height, 1);
    assert_eq!(decoded.color_format, ColorFormat::Rgba8888);
...
fn lossy_rgba_test() {
    ...
    let encoded = encode(ColorFormat::Rgba8888, false, 2, 2, &rgba).unwrap();
