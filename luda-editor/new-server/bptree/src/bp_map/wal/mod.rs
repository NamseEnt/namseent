...
impl Wal {
    ...
    pub(crate) fn update_pages(&mut self, pages: &BTreeMap<PageRange, PageBlock>) -> Result<()> {
        self.start_write()?;
        ...
        let put_page_blocks = PutPageBlocks::new(pages.iter());

        self.write_wal(put_page_blocks)?;
