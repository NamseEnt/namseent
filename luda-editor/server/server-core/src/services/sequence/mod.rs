...
impl rpc::SequenceService<SessionDocument> for SequenceService {
    ...
    > {
        Box::pin(async move {
            ...
            Ok(rpc::create_sequence::Response { sequence_id })
        })
    }
    ...
    // fn create_sequence_cut<'a>(
    //     &'a self,
    //     session: Option<SessionDocument>,
    //     rpc::create_sequence_cut::Request { sequence_id }: rpc::create_sequence_cut::Request,
    // ) -> std::pin::Pin<
    //     Box<dyn 'a + std::future::Future<Output = rpc::create_sequence_cut::Result> + Send>,
    // > {
    //     Box::pin(async move {
    //         let Some(session) = session else {
    //             return Err(rpc::create_sequence_cut::Error::Unauthorized);
    //         };

    //         let cut_id = rpc::Uuid::new_v4();

    //         crate::dynamo_db()
    //             .transact_with_cancel::<rpc::create_sequence_cut::Error>()
    //             .update_item(SequenceDocumentUpdate {
    //                 pk_id: sequence_id,
    ...
    //                         .await
    //                         .map_err(|error| {
    //                             rpc::create_sequence_cut::Error::Unknown(error.to_string())
    //                         })?;

    //                     if !is_project_editor {
    //                         return Err(rpc::create_sequence_cut::Error::Forbidden);
    //                     }

    //                     document.sequence.cuts.push(SequenceCutMeta {
    //                         id: cut_id,
    //                         version: 0,
    //                     });
    //                     Ok(document)
    //                 },
    //             })
    //             .create_item(SequenceCutDocument {
    //                 sequence_id,
    //                 id: cut_id,
    //                 version: 0,
    //                 cut: rpc::data::Cut::new(cut_id),
    //             })
    //             .send()
    //             .await
    //             .map_err(|error| match error {
    //                 crate::storage::dynamo_db::TransactError::Canceled(canceled) => canceled,
    //                 crate::storage::dynamo_db::TransactError::Unknown(unknown) => {
    //                     rpc::create_sequence_cut::Error::Unknown(unknown.to_string())
    //                 }
    //             })?;

    //         Ok(rpc::create_sequence_cut::Response { cut_id })
    //     })
    // }

    // fn delete_sequence_cut<'a>(
    //     &'a self,
    //     session: Option<SessionDocument>,
    //     rpc::delete_sequence_cut::Request {
    //         sequence_id,
    //         cut_id,
    //     }: rpc::delete_sequence_cut::Request,
    // ) -> std::pin::Pin<
    //     Box<dyn 'a + std::future::Future<Output = rpc::delete_sequence_cut::Result> + Send>,
    // > {
    //     Box::pin(async move {
    //         let Some(session) = session else {
    //             return Err(rpc::delete_sequence_cut::Error::Unauthorized);
    //         };

    //         crate::dynamo_db()
    //             .transact_with_cancel::<rpc::delete_sequence_cut::Error>()
    //             .update_item(SequenceDocumentUpdate {
    //                 pk_id: sequence_id,
    //                 update: move |mut document: SequenceDocument| async move {
    //                     let is_project_editor = crate::services()
    ...
    //                         .await
    //                         .map_err(|error| {
    //                             rpc::delete_sequence_cut::Error::Unknown(error.to_string())
    //                         })?;

    //                     if !is_project_editor {
    //                         return Err(rpc::delete_sequence_cut::Error::Forbidden);
    //                     }

    //                     document.sequence.remove_cut(cut_id);

    //                     Ok(document)
    //                 },
    //             })
    //             .delete_item(SequenceCutDocumentDelete {
    //                 pk_sequence_id: sequence_id,
    //                 sk_id: cut_id,
    //             })
    //             .send()
    ...
    //                 crate::storage::dynamo_db::TransactError::Canceled(canceled) => canceled,
    //                 crate::storage::dynamo_db::TransactError::Unknown(unknown) => {
    //                     rpc::delete_sequence_cut::Error::Unknown(unknown.to_string())
    //                 }
    //             })?;

    //         Ok(rpc::delete_sequence_cut::Response {})
    //     })
    // }
}
