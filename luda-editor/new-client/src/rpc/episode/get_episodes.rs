...
pub fn get_episodes<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
{
    server_rpc(ctx, request, dependencies, 12u16)
}
pub fn get_episodes_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
...
impl ServerConnection {
    pub async fn get_episodes<'a>(&'a self, request: RefRequest<'a>) -> Result<Response, Error> {
        self.request(
            12u16,
...
pub fn make_get_episodes_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(12u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
