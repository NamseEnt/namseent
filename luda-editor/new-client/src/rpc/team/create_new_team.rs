...
use crate::{network::*, *};
pub use luda_rpc::team::create_new_team::*;
use std::sync::{
    atomic::{AtomicBool, Ordering},
    Arc,
};
pub fn create_new_team<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
pub fn create_new_team_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
    ...
    Artifacts: StaticOwned,
{
    match create_new_team(ctx, request, dependencies).as_ref() {
...
impl ServerConnection {
    pub async fn create_new_team<'a>(&'a self, request: RefRequest<'a>) -> Result<Response, Error> {
        self.request(
            4u16,
...
pub fn make_create_new_team_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(4u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
