...
pub fn get_projects<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
{
    server_rpc(ctx, request, dependencies, 10u16)
}
pub fn get_projects_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
...
impl ServerConnection {
    pub async fn get_projects<'a>(&'a self, request: RefRequest<'a>) -> Result<Response, Error> {
        self.request(
            10u16,
...
pub fn make_get_projects_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(10u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
