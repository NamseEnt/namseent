...
pub fn google_auth<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
pub fn google_auth_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
...
impl ServerConnection {
    pub async fn google_auth<'a>(&'a self, request: RefRequest<'a>) -> Result<Response, Error> {
        self.request(
            0u16,
...
pub fn make_google_auth_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(0u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
