...
pub fn create_team_invite_code<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
pub fn create_team_invite_code_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
...
pub fn make_create_team_invite_code_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(6u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
