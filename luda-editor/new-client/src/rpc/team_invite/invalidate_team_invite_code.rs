...
use crate::{network::*, *};
pub use luda_rpc::team_invite::invalidate_team_invite_code::*;
use std::sync::{
    atomic::{AtomicBool, Ordering},
    Arc,
};
pub fn invalidate_team_invite_code<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
pub fn invalidate_team_invite_code_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
    ...
    Artifacts: StaticOwned,
{
    match invalidate_team_invite_code(ctx, request, dependencies).as_ref() {
...
impl ServerConnection {
    pub async fn invalidate_team_invite_code<'a>(
        &'a self,
...
pub fn make_invalidate_team_invite_code_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(8u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
