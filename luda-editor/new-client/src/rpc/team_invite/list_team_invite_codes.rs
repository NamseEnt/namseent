...
pub fn list_team_invite_codes<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
...
pub fn list_team_invite_codes_render<'a, Deps, Artifacts>(
    ctx: &'a RenderCtx,
    request: impl FnOnce(Deps) -> Option<(RefRequest<'a>, Artifacts)>,
    dependencies: Deps,
...
pub fn make_list_team_invite_codes_fn<'a, MakeRequestFn, ResponseFn, Artifacts>(
    ctx: &'a RenderCtx,
    ...
    (
        move || {
            ...
            tokio::spawn(async move {
                let result = server_connection.request(7u16, bytes).await;
                on_progress.store(false, Ordering::Relaxed);
