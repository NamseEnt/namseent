...
pub struct BackgroundTrack {
    pub id: String,
    pub clips: Arc<[Arc<BackgroundClip>]>,
}
#[derive(Debug)]
struct MovingChunk {
    clips: Vec<Arc<BackgroundClip>>,
    center_time: Time,
}
impl BackgroundTrack {
    pub(crate) fn get_clip_at_time(&self, time: &Time) -> Option<&BackgroundClip> {
    ...
    pub(crate) fn insert_clip(&mut self, clip: Arc<BackgroundClip>, at: Time) {
    ...
    fn convert_moving_clips_to_chunks<T: AsRef<str>>(
        ...
        let get_clip_index =
            |clip: &BackgroundClip| self.clips.iter().position(|c| c.id == clip.id).unwrap();
    ...
    pub(crate) fn resize_clip_delta(&mut self, clip_id: &str, delta_time: Time) {
        ...
        let new_clip = Arc::new(BackgroundClip {
            id: clip.id.clone(),
...
fn push_front_camera_clips(clips: &mut [Arc<BackgroundClip>]) {
