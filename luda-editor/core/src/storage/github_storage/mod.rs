...
pub struct GithubStorage<T: crdt::History> {
...
fn to_resource_path(path: &str) -> String {
    format!("{RESOURCES_PREFIX}/{path}")
}

impl<T: crdt::History> GithubStorage<T> {
    pub fn new(github_client: GithubClient, branch: String) -> Self {
        Self {
...
impl<T: crdt::History> Storage<T> for GithubStorage<T> {
    fn get<'a>(
        &'a self,
...
mod test {
    ...
    //     let github_storage = GithubStorage::new(mock_client, "master".to_string());
